...
int intSize() {
  int intArray[10];
  return (char*) &intArray[1] - (char*) intArray;
}
...
int doubleSize() {
  double doubArray[10];
  return (char*) &doubArray[1] - (char*) doubArray;
}
...
int pointerSize() {
  double * ptrArray[10];
  return (char*) &ptrArray[1] - (char*) ptrArray;
}
...
int withinArray(int * intArray, int size, int * ptr) {
  long start = (long) intArray;
  long end = (long) (intArray + size - 1);
  long i = (long) ptr;
  return ((i - start) >> 63 == 0) & ((end - i) >> 63 == 0);
}
/*
