...
void* mm_malloc (size_t size) {
  ...
  allocatedBlock = (BlockInfo*) searchFreeList(reqSize);
  ...
  allocatedBlockSize = SIZE(allocatedBlock->sizeAndTags);
  freeBlockSize = allocatedBlockSize - reqSize;

  if (freeBlockSize >= MIN_BLOCK_SIZE) {
    precedingBlockUseTag = allocatedBlock->sizeAndTags & TAG_PRECEDING_USED;
    allocatedBlock->sizeAndTags = reqSize | precedingBlockUseTag | TAG_USED; // new header

    // let's construct a new free block
    freeBlock = (BlockInfo*) POINTER_ADD(allocatedBlock, reqSize);
    freeBlock->sizeAndTags = freeBlockSize | TAG_PRECEDING_USED;  // header
    *((size_t*) POINTER_ADD(freeBlock, freeBlockSize - WORD_SIZE)) = freeBlock->sizeAndTags; // footer

    // and modify the use tag of the following block
    followingBlock = (BlockInfo*) POINTER_ADD(freeBlock, freeBlockSize);
    ...
    coalesceFreeBlock(freeBlock);
      ...
      allocatedBlock->sizeAndTags |= TAG_USED;
      followingBlock = (BlockInfo*) POINTER_ADD(allocatedBlock, allocatedBlockSize);
      followingBlock->sizeAndTags |= TAG_PRECEDING_USED;
  }

  removeFreeBlock(allocatedBlock);
  return (void*) POINTER_ADD(allocatedBlock, WORD_SIZE);
}
...
void mm_free (void *ptr) {
  ...
  block = (BlockInfo*) POINTER_SUB(ptr, WORD_SIZE);
  block->sizeAndTags &= ~TAG_USED;
  blockSize = SIZE(block->sizeAndTags);
  *((size_t*) POINTER_ADD(block, blockSize - WORD_SIZE)) = block->sizeAndTags;

  followingBlock = (BlockInfo*) POINTER_ADD(block, blockSize);
  followingBlock->sizeAndTags &= ~TAG_PRECEDING_USED;
